...
@fixture
def misc_init_herd_https(tmp_path: Path, misc_init_herd_https_session: Path) -> None:
    copy_directory(misc_init_herd_https_session, to=tmp_path)
    symlink = util.valid_clowder_symlink(tmp_path)
    os.unlink(symlink)
...
def cats_init_herd_default_session(tmp_path_factory) -> Path:
    ...
    util.run_command(f"clowder init {util.get_url('cats', 'https')}", path, check=True)
    util.run_command("clowder herd", path, check=True)
...
def misc_init_herd_default_session(tmp_path_factory) -> Path:
    ...
    util.run_command(f"clowder init {util.get_url('misc', 'https')}", path, check=True)
    util.run_command("clowder herd", path, check=True)
...
@fixture(scope="session")
def misc_init_herd_https_session(tmp_path_factory) -> Path:
    path = tmp_path_factory.mktemp(misc_init_herd_default_session.__name__)

    util.run_command(f"clowder init {util.get_url('misc', 'https')}", path, check=True)
    util.run_command(f"clowder link https", path, check=True)
    util.run_command("clowder herd", path, check=True)
...
def cats_init_default_session(tmp_path_factory) -> Path:
    ...
    util.run_command(f"clowder init {util.get_url('cats', 'https')}", tmp_path, check=True)
...
def misc_init_default_session(tmp_path_factory) -> Path:
    ...
    util.run_command(f"clowder init {util.get_url('misc', 'https')}", tmp_path, check=True)
