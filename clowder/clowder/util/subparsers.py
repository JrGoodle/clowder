...
def _configure_subparser_clean(subparsers, clowder):
    ...
    parser_clean.add_argument('--all', '-a', action='store_true', help='clean all the things')

    parser_clean.add_argument('--recursive', '-r', action='store_true', help='clean submodules recursively')

    parser_clean.add_argument('-d', action='store_true', help='remove untracked directories')

    parser_clean.add_argument('-f', action='store_true', help='remove directories with .git subdirectory or file')

    parser_clean.add_argument('-X', action='store_true', help='remove only files ignored by git')

    parser_clean.add_argument('-x', action='store_true', help='remove all untracked files')
...
def _configure_subparser_forall(subparsers, clowder):
    ...
    parser_forall.add_argument('--parallel', action='store_true', help='run commands in parallel')

    parser_forall.add_argument('--ignore-errors', '-i', action='store_true', help='ignore errors in command or script')
...
def _configure_subparser_herd(subparsers, clowder):
    ...
    parser_herd.add_argument('--depth', '-d', default=None, type=int, nargs=1, metavar='DEPTH', help='depth to herd')

    group_herd = parser_herd.add_mutually_exclusive_group()

    group_herd.add_argument('--branch', '-b', nargs=1, default=None, metavar='BRANCH', help='branch to herd if present')

    group_herd.add_argument('--tag', '-t', nargs=1, default=None, metavar='TAG', help='tag to herd if present')
...
def _configure_subparser_init(subparsers):
    ...
    parser_init.add_argument('--branch', '-b', nargs=1, metavar='BRANCH', help='branch of repo containing clowder.yaml')
...
def _configure_subparser_prune(subparsers, clowder):
    ...
    group_prune_options.add_argument('--all', '-a', action='store_true', help='prune local and remote branches')

    group_prune_options.add_argument('--remote', '-r', action='store_true', help='prune remote branches')
...
def _configure_subparser_repo(subparsers):
    ...
    parser_repo_add.add_argument('files', nargs='+', metavar='FILE', help='files to add')
    ...
    parser_repo_checkout.add_argument('ref', nargs=1, metavar='REF', help='git ref to checkout')
    ...
    parser_repo_commit.add_argument('message', nargs=1, metavar='MESSAGE', help='commit message')
    ...
    parser_repo_run.add_argument('command', nargs=1, metavar='COMMAND', help=repo_run_command_help)
...
def _configure_subparser_reset(subparsers, clowder):
    ...
    parser_reset.add_argument('--parallel', action='store_true', help='run commands in parallel')
...
def _configure_subparser_start(subparsers, clowder):
    ...
    parser_start.add_argument('--tracking', '-t', action='store_true', help='create remote tracking branch')
...
def _configure_subparser_status(subparsers):
    ...
    parser_status.add_argument('--fetch', '-f', action='store_true', help='fetch projects before printing status')
...
def _configure_subparser_sync(subparsers, clowder):
    ...
    parser_sync.add_argument('--parallel', action='store_true', help='run commands in parallel')

    parser_sync.add_argument('--rebase', '-r', action='store_true', help='use rebase instead of pull')
...
def _configure_subparser_yaml(subparsers):
    ...
    parser_yaml.add_argument('--resolved', '-r', action='store_true', help='print resolved clowder.yaml')
